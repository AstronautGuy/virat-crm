...
const TARGET_URL =
  process.env.EXPO_PUBLIC_API_URL || "https://virat-crm.vercel.app";
...
async function syncLocationQueue() {
  try {
    ...
    const response = await fetch(
      `${TARGET_URL}/api/trpc/location.logBreadcrumbBatch`,
      {
        method: "POST",
        credentials: "include",
...
TaskManager.defineTask(NOTIFICATION_TASK_NAME, async () => {
  try {
    const response = await fetch(
      `${TARGET_URL}/api/trpc/notifications.getUnreadCount`,
      {
        method: "GET",
        credentials: "include",
      },
      ...
      try {
        ...
        try {
          const res = await fetch(
            `${TARGET_URL}/api/trpc/location.getServerTime`,
            {
              method: "GET",
              credentials: "include",
            },
        ...
        console.warn(
          "Background location setup failed (expected if running in Expo Go):",
          e,
        );
      }
    })();
      ...
      {isLockedOut && (
        <View style={styles.lockoutOverlay}>
          ...
          <Text style={styles.lockoutText}>
            Location tracking failed for over 10 minutes. Please restore
            network/GPS connectivity.
          </Text>
          <TouchableOpacity

...
async function run() {
  ...
  await page.goto("http://localhost:3000/admin/reports", { waitUntil: "domcontentloaded", timeout: 60000 });
  ...
  await page.waitForSelector("#export-sales-pdf", { visible: true, timeout: 60000 });
  await page.click("#export-sales-pdf");
  
  await new Promise((r) => setTimeout(r, 3000));

  console.log("Clicking Export Attendance PDF...");
  await page.waitForSelector("#export-attendance-pdf", { visible: true, timeout: 60000 });
  ...
  const blobs: Record<string, string> = await page.evaluate(() => (window as any).downloadedBlobs);
  let fileCount = 1;

...
export const maintenanceRouter = createTRPCRouter({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (let i = 0; i <= input.months; i++) {
        ...
        const mergedStats = new Map<
          number,
          { revenue: number; salesCount: number; totalQty: number }
        >();
        ...
        for (const s of archiveStats) {
          const existing = mergedStats.get(s.branchId) ?? {
            revenue: 0,
            salesCount: 0,
            totalQty: 0,
          };
        ...
        for (const [branchId, metrics] of mergedStats.entries()) {
          await ctx.db
            ...
            .onConflictDoUpdate({
              target: [
                performanceSnapshots.entityType,
                performanceSnapshots.entityId,
  ...
  archiveStaleData: featureProtectedProcedure("admin").mutation(
    async ({ ctx }) => {
      ...
      return await ctx.db.transaction(async (tx) => {
        ...
        const toArchive = await tx
          .select({ id: sales.id })
          .from(sales)
          .where(lt(sales.createdAt, oneYearAgo));
        ...
        await tx
          .delete(replacements)
          .where(inArray(replacements.originalSaleId, ids));

...
export const authRouter = createTRPCRouter({
  login: publicProcedure
    .meta({ openapi: { method: "POST", path: "/auth/login", summary: "Login and get token", tags: ["Auth"] } })
    .input(z.object({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const permissionsMap = perms.reduce((acc, p) => {
      ...
      const token = await encrypt({ userId: user.id, expires: expires.toISOString() });

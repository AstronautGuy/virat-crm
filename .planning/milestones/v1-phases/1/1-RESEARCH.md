...
- **Usage for conditional UI**:
  ...
  export default async function Layout({ children }) {
    ...
    return <nav>{adminPermission?.isGranted && <AdminButton />}</nav>;
  }
  ```

- **Note on Permissions vs Roles**: Kinde recommends checking _permissions_ (e.g., `admin:access`) rather than raw roles. However, if the raw role is needed, `getClaim('roles')` can be used.
